...
var unsolicitedEvents = {
  ...
  // Debugger (thread actor) events. These are always unsolicited, so they
  // must be routed as events even when a request to the thread actor is in
  // flight. Note "paused"/"resumed" are intentionally absent: they double as
  // the replies to attach/interrupt/resume and must be delivered as responses.
  "newSource": "newSource",
  "newGlobal": "newGlobal"
};
