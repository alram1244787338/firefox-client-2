...
Webapps.prototype = extend(ClientMethods, {
  ...
  getInstalledApps: function(cb) {
    this.request("getAll", this.pluck('apps'), cb);
  },
  listRunningApps: function(cb) {
    this.request("listRunningApps", this.pluck('apps'), cb);
  },
  getApp: function(manifestURL, cb) {
    this.request("getAppActor", {manifestURL: manifestURL},
                 this.wrapForm('actor', Tab), cb);
  },
